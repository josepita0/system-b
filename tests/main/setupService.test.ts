...
describe('SetupService', () => {
  it('requires the wizard for a fresh bootstrap installation', () => {
    ...
    expect(status.bootstrapDisplay).not.toBeNull()
    expect(status.bootstrapDisplay?.username).toBe('admin')
    expect(status.bootstrapDisplay?.temporaryPassword.length).toBeGreaterThan(0)
    expect(status.bootstrapDisplay?.recoveryCodes.length).toBeGreaterThan(0)
  ...
  it('marks installation as completed after password change', () => {
    ...
    expect(status.bootstrapDisplay).toBeNull()
